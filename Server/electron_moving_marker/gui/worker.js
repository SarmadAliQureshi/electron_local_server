...
serverUrl = _url + folder
console.log('test1', serverUrl)
return serverUrl

}

function fetchServer(){
var a = document.getElementById('serverurl').value
...
let options1 = {
    ...
    PythonShell.run('Server/electron_moving_marker/csv to srt/get_Url.py',options1,  function  (err, api_results)  {
    if  (err)  throw err;
...
function databaseFields(){
    ...
    PythonShell.run('Server/electron_moving_marker/csv to srt/Db_check.py',options1_22,  function  (err, results)  {
...
//})
     ...
     var check = document.getElementById('serverurl').value
    if (check == 'server'){
...
//    alert('SRT Files Inserted')
    ...
    var mylist = []
    if (err) throw err;
    var txtline = data.toString().split(',')
    for (var i=0;i<txtline.length;i++){
        var optns = txtline[i].replace('[','').replace(']','').replace("'",'').replace("'",'')
//        optns=optns.split('/')
//        optns=optns[optns.length-1]
...
//        var opt = document.createElement('option');
    ...
    PythonShell.run('Server/electron_moving_marker/electron_moving_tracks/electron_moving_tracks/track_api.py',options1,  function  (err, api_results)  {
    if  (err)  throw err;
    console.log('API results');
    console.log('API results', api_results);
    });
...
//    var videoNameGet = video_name_split[0]
    video_name_split=video_name_split[video_name_split.length-1]
    video_name=serverUrl+video_name_split
